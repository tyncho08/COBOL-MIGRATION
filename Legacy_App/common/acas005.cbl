...
*> Error Messages Used.
*>
*>                      GL901 Note error and hit return
*>                      GL902 Program Error: Temp rec =yyy < NL-Rec = zzz
*>**
...
*> 04/01/17 vbc -     Taken from irsnominal removing sub nom. processing.
*> 28/02/18 vbc - .07 Added recs test for FH at ba012-Test-WS-Rec-Size-2
*>                    and removed msgs 906 & 7 renamed IR901/2 to GL901/2.
*> 29/04/18 vbc - .08 Resetting Cobol-File-Status for write,rewrite,delete, start.
...
*>    acas005 ONLY          WS-Ledger-Record = Contents of data record to be written/read
...
*>**********************
*>
 77  prog-name          pic x(20)  value "acas005 (3.02.10)".
*>
...
*> Module Specific
     ...
     03  GL902          pic x(32) value "GL902 Program Error: Temp rec = ".
*>                                        yyy < NL-Rec = zzz
*>
 Linkage Section.
*>**************
...
*>     special Delete-All instead. -- NOT used with GL.
*>
 *>    if       fn-Open and
 *>             fn-output
 ...
 aa020-Process-Open.
     ...
     if       fn-input
       ...
       if     fn-i-o
              ...
              if       fs-reply not = zero
                       close       Ledger-File
                       open output Ledger-File           *> Doesnt create in i-o
                       close       Ledger-File
                       open i-o    Ledger-File
              end-if
       else
        if    fn-output
              open output Ledger-File         *> caller should check fs-reply
        else
...
*>              open extend Ledger-File
     ...
     move     "OPEN GL NL File" to WS-File-Key
     if       fs-reply not = zero
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     move     zero to Cobol-File-Status.
     move     "CLOSE GL NL File" to WS-File-Key.
...
*>
 aa041-Reread.
     ...
     if       FS-Reply not = zero
              go to aa999-main-exit.
     move     Ledger-Record to  WS-Ledger-Record.
     move     Ledger-Key   to WS-File-Key.
     move     zeros to WE-Error.
     go to    aa999-main-exit.
*>
*>   The next block will never get executed unless performed
*>
 aa047-Eval-Keys.
...
*>
     if       fn-equal-to
              start Ledger-File key = Ledger-Key invalid key
                    move 21 to Fs-Reply
            move 255 to WE-Error        *> USED by  GL?
                    go to aa999-main-exit
              end-start
     end-if
     if       fn-not-less-than
...
*>
 aa070-Process-Write.
     ...
     move     WS-Ledger-Record to Ledger-Record.
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
     move     Ledger-Key to WS-File-Key.
     write    Ledger-Record invalid key
              move 22 to FS-Reply
     end-write.
     move     WS-Ledger-Key to WS-File-Key.
     go       to aa999-main-exit.
*>
 aa080-Process-Delete.
     ...
     move     WS-Ledger-Key to Ledger-Key
                               WS-File-Key.
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
     delete   Ledger-File record invalid key
              move 21 to FS-Reply
     end-delete
     go       to aa999-main-exit.
*>
...
*>
     if       A = zero                        *> so it is being called first time
              ...
              if       WE-Error = 901                  *> record length wrong so display error, accept and then stop run.
                       ...
                       string GL902          delimited by size
                              A              delimited by size
                              " < "          delimited by size
                              "NL-Rec = " delimited by size
