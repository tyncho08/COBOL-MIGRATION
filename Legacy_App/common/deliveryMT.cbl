...
*>**
*> Remarks.             Delivery File RDB Handler using amended JC preSQL.
*>                      **********************************************
*>
...
*>**
*> Called by Modules:
*>                      acas014 - delivery Cobol Handler for sales & purchase.
*>
*>**
*> Error Messages Used.
...
*>                    No changes needed for DELIVERYLD.
*>                .10 Force logging at end of rewrite process - missed.
*>                    On rewrite not setting DELIV-KEY for logging.
*> 08/01/17 vbc -     Taken from glpostingMT.
*> 01/03/18 vbc - .11 Renamed error messages to SM901, SM004 as needed.
*> 13/03/18 vbc - .12 Renamed DELIVERY-RECORD to DELIVERY-REC same as all the
*>                    other tables (-REC).
...
*>       for both FS-Reply and WE-Error and table used - in this case the SL  Delivery File.
...
*>
 ...
 77  prog-name           pic x(22)    value "deliveryMT (3.02.12)".
*>
...
*>
*> Metadata on primary and alternate keys...  from Prima DAL for DELIVERY-REC
*>
 01  Table-Of-Keynames.
     03  filler          pic x(30) value "DELIV-KEY                     ". *> in RDB
...
*> /MYSQL VAR\
*>       ACASDB
*>       TABLE=DELIVERY-REC,HV
 COPY "mysql-variables.cpy".
*>
*>    Definitions for the DELIVERY-REC Table
*>
       01  TP-DELIVERY-REC                       USAGE POINTER.
       01  TD-DELIVERY-REC.
           05  HV-DELIV-KEY                      PIC X(8).
           05  HV-DELIV-NAME                     PIC X(30).
           05  HV-DELIV-ADDRESS                  PIC X(96).
*> /MYSQL-END\
*>
 Linkage Section.
*>**************
...
*> *> /MYSQL-END\
*>
     move    "OPEN DELIVERY" to WS-File-Key
...
*>
     ...
     move    "CLOSE DELIVERY" to WS-File-Key.
*>  /MYSQL CLOSE\
*>
*>    Close the Database
*>
...
*>                    TABLE=DELIVERY-REC
           ...
           STRING "SELECT * FROM "
             "`DELIVERY-REC`"
             " WHERE "
             ws-Where (1:J)      *> DELIV-KEY > "00000000" ORDER BY DELIVERY-REC ASC
           ...
           MOVE WS-MYSQL-RESULT TO TP-DELIVERY-REC
*>               /MYSQL-END\
...
*>
*>             TABLE=DELIVERY-REC
           MOVE TP-DELIVERY-REC TO WS-MYSQL-RESULT
...
*>
     if       WS-MYSQL-Count-Rows = zero
              ...
              if    WS-MYSQL-Error-Number (1:1) not = "0"
                    ...
                    initialize WS-Delivery-Record with filler
...
*>
     move     HV-DELIV-KEY to WS-File-Key.
...
*>
     ...
     string   "`"                   delimited by size
              ...
              "`"                   delimited by size
              '="'                  delimited by size
              WS-Delivery-Record (K:L)  delimited by size
 *>             DELIV-KEY
              '"'                   delimited by size
                      into WS-Where
                        with pointer J
...
*>
     ...
     move     WS-Where (1:J)  to WS-Log-Where.    *>  For test logging
     move     WS-Delivery-Record (K:L) to WS-File-Key.
     if    Testing-2
           display Display-Message-1 with erase eos
     end-if
*>
...
*>
 ba070-Process-Write.
     perform  bb000-HV-Load.                       *>  move WS-Delivery-Record fields to HV fields
     move     ws-DELIV-KEY to WS-File-Key.
...
*>
     move     "ZZZZZZZZ"  to ws-DELIV-KEY.         *> as its the last delivery
*>
     set      KOR-x1 to 1
     move     KOR-offset (KOR-x1) to K
     move     KOR-length (KOR-x1) to L
*>
     ...
     string   "Deleting back from " delimited by size
               ws-DELIV-KEY             delimited by size
                      into WS-File-Key
...
*>
     initialize TD-DELIVERY-REC.
     move     WS-Deliv-Key      to HV-DELIV-KEY.
     move     Deliv-Name        to HV-DELIV-NAME.
     move     Deliv-Address     to HV-DELIV-ADDRESS.
*>
...
*>
     initialize WS-Delivery-Record.
     move     HV-DELIV-KEY       to WS-Deliv-Key.
     move     HV-DELIV-NAME      to Deliv-Name.
     move     HV-DELIV-ADDRESS   to Deliv-Address.
*>
 bb100-Exit.
     exit section.
*>
...
*>
           STRING '`DELIV-KEY`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-DELIV-KEY,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`DELIV-NAME`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-DELIV-NAME,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ', 'INTO WS-MYSQL-COMMAND
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`DELIV-ADDRESS`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-DELIV-ADDRESS,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ";" INTO WS-MYSQL-COMMAND
