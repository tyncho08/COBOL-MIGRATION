...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the posting    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 postingRES source taken from analysisRES.
...
*>
 copy "selpost.cob".
     select  posting-File-Seq  assign        "posting.seq"
...
*>----------------------
 77  prog-name           pic x(22)    value "glpostingRES (3.02.00)".
...
*>
 aa000-Main-Start.
     ...
     move     2      to WS-Log-System.   *> 1 = IRS, 2=GL, 3=SL, 4=PL, 5=Stock used in FH logging, 6=SL & PL
...
*>
     open     input    posting-File-seq.
     if       FS-Reply not = zeros
              move     "No Posting file to process" to SO-Print
              perform  Call-Sysout
              close    posting-File
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open - Posting file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     posting-File-seq next record at end
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_postingS now
*>
 Call-Sysout.
