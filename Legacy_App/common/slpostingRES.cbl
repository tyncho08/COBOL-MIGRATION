...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the slposting    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 slpostingRES source taken from analysisRES.
...
*>
 copy "selpost-irs.cob".
     select  IRS-Post-File-Seq  assign          "postings2irs.seq"
...
*>
 copy "fdpost-irs.cob".
 fd  IRS-Post-File-Seq.
 copy "wspost-irs.cob".
*>
 working-storage section.
*>----------------------
 77  prog-name           pic x(24)    value "slpostingRES (3.02.00)".
...
*>
 01  Error-Messages.
     03  SY003          pic x(41) value "SY003 Problem reading slposting file = ".
...
*>
     open     input    IRS-Post-File-Seq.
     if       FS-Reply not = zeros
              move     "No slposting file to process" to SO-Print
              perform  Call-Sysout
              close    IRS-Post-File
...
*>
 aa010-Read-Recs.
     perform  forever
              read     IRS-Post-File-Seq next record at end
              ...
              write    IRS-Posting-Record from WS-IRS-Posting-Record
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_slpostingS now
*>
 Call-Sysout.
