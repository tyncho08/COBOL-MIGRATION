...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the slautogen    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 slautogenRES source taken from analysisRES.
...
*>
 copy "slSELautogen.cob".
     select  Agen-Invoice-File-Seq  assign    "slautogen.seq"
...
*>
     open     input    Agen-Invoice-File-Seq.
     if       FS-Reply not = zeros
              move     "No slautogen file to process" to SO-Print
              perform  Call-Sysout
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open slautogen file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     Agen-Invoice-File-Seq next record at end
              ...
              write    Agen-Invoice-Record from WS-Invoice-Record
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_slautogenS now
*>
 Call-Sysout.
