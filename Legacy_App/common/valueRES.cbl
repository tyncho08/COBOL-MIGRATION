...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the value    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 valueRES source taken from analysisRES.
*> 16/04/24 vbc       Copyright notice update superseding all previous notices.
*> 22/03/25 vbc - .01 Adjust sy003 / 6 msgs.
*>**
*>
*>*************************************************************************
*>
...
*>
 copy "selval.cob".
     select  Value-File-Seq  assign          "value.seq"
...
*>----------------------
 77  prog-name           pic x(24)    value "valueRES (3.02.01)".
...
*>
 01  Error-Messages.
     ...
     03  SY006          pic x(31) value "SYS06 Write error on .dat file ".
...
*>
     open     input    Value-File-Seq.
     if       FS-Reply not = zeros
              move     "No value Seq file to process" to SO-Print
              perform  Call-Sysout
              close    Value-File-Seq
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open Value file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     Value-File-Seq next record at end
              ...
              write    Value-Record from WS-Value-Record
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_valueS now
*>
 Call-Sysout.
