...
*>**
*> Remarks.             delinvnos File RDB Handler using amended JC preSQL.
*>                      **************************************************
*>
...
*>                      from the ACAS
*>                      delinvnos file handler acas017
...
*>**
*> Called by Modules:
*>                      acas017 - delinvnos Cobol Handler for sales & purchase.
*>
*>**
*> Error Messages Used.
...
*>       for both FS-Reply and WE-Error and table used - in this case the SL  delinvnos File.
...
*>
 ...
 77  prog-name           pic x(23)    value "sldelinvnosMT (3.02.11)".
*>
...
*>
*> Metadata on primary and alternate keys...  from Prima DAL for SADELINV-REC
*>
...
*> /MYSQL VAR\
*>       ACASDB
*>       TABLE=SADELINV-REC,HV
 COPY "mysql-variables.cpy".
*>
*>    Definitions for the SADELINV-REC Table
*>
       01  TP-SADELINV-REC                       USAGE POINTER.
       01  TD-SADELINV-REC.
...
*> *> /MYSQL-END\
*>
     move    "OPEN DELINVNOS" to WS-File-Key
...
*>
     ...
     move    "CLOSE DELINVNOS" to WS-File-Key.
*>  /MYSQL CLOSE\
*>
*>    Close the Database
*>
...
*>                    TABLE=SADELINV-REC
           ...
           STRING "SELECT * FROM "
             "`SADELINV-REC`"
             " WHERE "
             ws-Where (1:J)      *> DEL-INV-NOS > "00000000" ORDER BY SADELINV-REC ASC
           ...
           MOVE WS-MYSQL-RESULT TO TP-SADELINV-REC
*>               /MYSQL-END\
...
*>
*>             TABLE=SADELINV-REC
           MOVE TP-SADELINV-REC TO WS-MYSQL-RESULT
...
*>
     move     99999999  to ws-DEL-INV-NOS.         *> as its the last delinvnos
*>
     set      KOR-x1 to 1
     move     KOR-offset (KOR-x1) to K
     move     KOR-length (KOR-x1) to L
