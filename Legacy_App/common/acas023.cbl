...
*>    acas023 ONLY          WS-Delfolio-Record = Contents of data record to be written/read
...
*>
 ...
 file-control.
*>
*> taken from copybooks now but used in sales & purchase with diff. FN.
*>
 copy "selpdnos.cob".
*>
 data division.
...
*>**********************
*>
 77  prog-name          pic x(20)  value "acas023 (3.02.09)".
*>
...
*> Module Specific
     ...
     03  PL906          pic x(32) value "PL906 Program Error: Temp rec = ".
*>                                        yyy < Delfolio-Rec = zzz
*>
 Linkage Section.
*>**************
 copy "wsdnos.cob".
...
*>              open extend Del-Inv-Nos-File
     ...
     move     "OPEN DelFolio File" to WS-File-Key.
     if       fs-reply not = zero
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     move     zero to Cobol-File-Status.
     move     "CLOSE DelFolio File" to WS-File-Key.
...
*>
     if       A = zero                        *> so it is being called first time
              ...
              if       WE-Error = 901                  *> record length wrong so display error, accept and then stop run.
                       ...
                       string PL906          delimited by size
                              A              delimited by size
                              " < "          delimited by size
...
*>
 ba020-Process-DAL.
     call     "delfolioMT" using File-Access
                                   ACAS-DAL-Common-data
