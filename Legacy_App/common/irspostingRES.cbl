...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the irsposting    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 irspostingRES source taken from analysisRES.
...
*>
     select  Posting-File    assign   File-36      *>  "post.dat"
                             ...
                             status   fs-reply.
*>
     select  irsposting-File-Seq  assign        "irspost.seq"
...
*>
 fd  irsposting-File-Seq.
 copy "irswspost.cob" replacing Posting-Record by WS-Posting-Record.
*>
 working-storage section.
*>----------------------
 77  prog-name           pic x(23)    value "irspostingRES (3.02.00)".
...
*>
     open     input    irsPosting-File-seq.
     if       FS-Reply not = zeros
              move     "No irsposting file to process" to SO-Print
              perform  Call-Sysout
              close    Posting-File
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open - IRS Posting file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     irsPosting-File-seq next record at end
              ...
              write    Posting-Record from WS-Posting-Record
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_irspostingS now
*>
 Call-Sysout.
