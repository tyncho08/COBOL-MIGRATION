...
*>   Written.            31 May 2023.
*>**
*>   Remarks.            This module only uses the batch     file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 glbatchRES source taken from analysisRES.
...
*>
 copy "selbatch.cob".
     select  glbatch-File-Seq  assign        "batch.seq"
...
*>----------------------
 77  prog-name           pic x(20)    value "glbatchRES (3.02.00)".
...
*>
     open     input    glBatch-File-Seq.
     if       FS-Reply not = zeros
              move     "No GLbatch file to process" to SO-Print
              perform  Call-Sysout
              close    Batch-File
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open - GL Batch file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     glBatch-File-Seq next record at end
              ...
              write    Batch-Record from WS-Batch-Record
