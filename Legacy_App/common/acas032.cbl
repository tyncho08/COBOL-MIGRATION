...
*>**
*> Remarks.             PL Payments File Handler.
*>                      **************
...
*> 11/01/18 vbc - .06 Changed WS-Log-File-No from 13 to 16/26.
...
*>    acas032 ONLY          WS-Pay-Record = Contents of data record to be written/read
...
*>**********************
*>
 77  prog-name          pic x(20)  value "acas032 (3.02.10)".
*>
...
*> Module Specific
     ...
     03  PL909          pic x(32) value "PL909 Program Error: Temp rec = ".
*>                                        yyy < Payments-Rec = zzz
*>
 Linkage Section.
*>**************
 copy "wspay.cob".
...
*>
     ...
     move     16     to WS-Log-File-No.  *> Cobol/RDB, File/Table within sub System.
*>
...
*>              open extend Pay-File
     ...
     move     "OPEN Payment File" to WS-File-Key.
     if       fs-reply not = zero
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     move     zero to Cobol-File-Status.
     move     "CLOSE Payment File" to WS-File-Key.
...
*>
 aa041-Reread.
     read     Pay-File next record at end
     ...
     if       FS-Reply not = zero
              go to aa999-main-exit.
     move     Pay-Record to  WS-Pay-Record.
     move     Pay-Key   to WS-File-Key.
...
*>
 aa051-Reread.
     move     WS-Pay-Key to Pay-Key WS-File-Key.
...
*>
     if       fn-equal-to
              start Pay-File key = Pay-Key invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
     if       fn-not-less-than
              start Pay-File key not < Pay-Key invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
...
*>
 aa070-Process-Write.
     ...
     move     WS-Pay-Record to Pay-Record.
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
     move     Pay-Key to WS-File-Key.
     write    Pay-Record invalid key
              go to  aa999-main-exit.   *> logging
     move     Pay-Key to WS-File-Key.
     go       to aa999-main-exit.
*>
...
*>
     move     26 to WS-Log-File-no.        *> for FHlogger
*>
...
*>
     if       A = zero                        *> so it is being called first time
              ...
              if       WE-Error = 901                  *> record length wrong so display error, accept and then stop run.
                       ...
                       string PL909          delimited by size
                              A              delimited by size
                              " < "          delimited by size
                              "Payments-Rec = " delimited by size
...
*>
 ba020-Process-DAL.
     call     "paymentsMT" using File-Access
                                 ACAS-DAL-Common-data
