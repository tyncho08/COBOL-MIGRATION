...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the Audit     file.
*>
...
*>**
*>   Changes.
*> 31/05/23 vbc - .00 auditUNL source taken from analysisUNL.
...
*>
 copy "selaud.cob".
     select  Audit-File-Seq  assign        "staudit.seq"
...
*>----------------------
 77  prog-name           pic x(20)    value "auditUNL (3.02.00)".
...
*>
     open     input    Audit-File-Seq.
     if       FS-Reply not = zeros
              move     "No STaudit file to process" to SO-Print
              perform  Call-Sysout
              close    Stock-Audit
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open - STaudit file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     Audit-File-Seq next record at end
              ...
              write    Stock-Audit-Record from WS-Stock-Audit-Record
