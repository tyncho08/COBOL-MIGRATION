...
*>        NEEDS TO BE DRY CHECKED - records areas in and OUT
*>********************************************
*>                                           *
*>  PLautogen Table Load via the PLAutogen   *
*>                  file                     *
*>                 for MySQL                 *
*>   code from pl InvoiceLD                  *
*>********************************************
*>
 identification division.
 program-id.             plautogenLD.
*>**
...
*>**
*>   Remarks.            PL autogen RDB Table load from the PL autogen File
...
*>**
*>   Called Modules.     acas030   } For file and table access.
*>                       PLautogenMT }
*>                       fhlogger
*>
*>**
...
*>**
*>   Changes.
*> 14/05/23 vbc - .00 Code imported from PLinoiceLD and changelog notes
*>                    cleared down.
...
*>----------------------
 77  prog-name           pic x(22)    value "plautogenLD (3.02.02)".
...
*> Program specific
     ...
     03  PL911          pic x(41) value "PL911 Error on PLautogenMT processing".
     03  PL912          pic x(38) value "PL912 Error opening PLautogen File = ".
     03  PL913          pic x(30) value "PL913 Error on reading file = ".
     03  PL914          pic x(31) value "PL914 Error on writing table = ".
     03  PL915          pic x(38) value "PL915 Error opening PLautogen Table = ".
     03  PL918          pic x(22) value "PL918 RDBMS details : ".
     03  PL919          pic x(24) value "PL919 No Data - aborting".
     03  PL997          pic x(32) value "PL997 No PLautogen file present".
...
*>
 aa000-main-start.
     ...
     move     17     to WS-Log-File-No.  *> Cobol/RDB, File/Table within sub System.17=PLinvoice
*>
...
*>
 aa010-Proc-Override.
*>
*>  Over-ride processes for acas030/PLautogenMT for Dup modes
*>
...
*>
     ...
     perform  acas030-Open-Input.      *> open input Cobol PLautogen FILE
     if       FS-Reply = 35
              move Prog-Name to SO-Print.
              perform call-Sysout
     ...
     perform  acas030-Open-Output.              *> Open/Init RDB via FH - PLautogen
     if       FS-Reply not = zero
     ...
     if       Sql-State = "23000"  *> Dup key (rec already present)
         ...
         or   SQL-Err (1:4) = "1062"
              ...
              move    zeros to FS-Reply
                               WE-Error
              perform acas030-Rewrite  *> in case it has been rerun.
              if      FS-Reply not = zero
                      move spaces to SO-Print
                      string  PL914
...
*>
 aa999-Finish.
     ...
     perform  acas030-Close.                     *> Close RDB
     close    System-File.
     move     zero to File-System-Used
                   FA-File-System-Used.
     perform  acas030-close.    *> Close cobol PLautogen file
     move     "EOJ - Load PLAutogen Table" to So-Print.
     perform  Call-Sysout.
     move     "CLOSE   " to SO-Print.
     perform  Call-Sysout.
     goback.
