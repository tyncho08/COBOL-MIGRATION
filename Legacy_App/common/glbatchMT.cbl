...
*>**
*> Remarks.             (GL) Batch File RDB Handler using amended JC preSQL.
*>                      **********************************************
*>
...
*>**
*> Changes.
*> 15/11/16 vbc - .06 Taken from irsnominalMT & modified for GL  processing & based on
*>                    irsub4.
...
*>                    No changes needed for GLBATCHLD.
*>                .10 Force logging at end of rewrite process - missed.
*>                    On rewrite not setting BATCH-KEY for logging.
*> 08/01/17 v bc -    Taken from glpostingMT.
...
*>       for both FS-Reply and WE-Error and table used - in this case the GL  batch File.
...
*>
 ...
 77  prog-name           pic x(22)    value "glbatchMT (3.02.11)".
*>
...
*>
*> Metadata on primary and alternate keys...  from Prima DAL for GLBATCH-REC
*>
*>  WARNING BATCH-KEY MAY WELL NEED CHANGING TO POST-RRN & RDB made to index fld.
*>
 01  Table-Of-Keynames.
     03  filler          pic x(30) value "BATCH-KEY                     ". *> in RDB
...
*>
*>    Definitions for the GLBATCH-REC Table
*>
       01  TP-GLBATCH-REC                        USAGE POINTER.
       01  TD-GLBATCH-REC.
           05  HV-BATCH-KEY                      PIC  9(08) COMP.
           05  HV-ITEMS                          PIC  9(03) COMP.
           05  HV-BATCH-STATUS                   PIC  9(03) COMP.
           05  HV-CLEARED-STATUS                 PIC  9(03) COMP.
           05  HV-BCYCLE                         PIC  9(03) COMP.
           05  HV-ENTERED                        PIC  9(10) COMP.
           05  HV-PROOFED                        PIC  9(10) COMP.
           05  HV-POSTED                         PIC  9(10) COMP.
           05  HV-STORED                         PIC  9(10) COMP.
           05  HV-INPUT-GROSS                    PIC  9(12)V9(02) COMP.
           05  HV-INPUT-VAT                      PIC  9(12)V9(02) COMP.
           05  HV-ACTUAL-GROSS                   PIC  9(12)V9(02) COMP.
           05  HV-ACTUAL-VAT                     PIC  9(12)V9(02) COMP.
           05  HV-DESCRIPTION                    PIC X(24).
           05  HV-BDEFAULT                       PIC  9(03) COMP.
           05  HV-CONVENTION                     PIC X(2).
           05  HV-BATCH-DEF-AC                   PIC  9(08) COMP.
           05  HV-BATCH-DEF-PC                   PIC  9(03) COMP.
           05  HV-BATCH-DEF-CODE                 PIC X(2).
           05  HV-BATCH-DEF-VAT                  PIC X(1).
           05  HV-BATCH-START                    PIC  9(08) COMP.
*> /MYSQL-END\
*>
 Linkage Section.
*>**************
...
*>  ALSO NEED TO CHECK FOR NEED OF DELETE-ALL & file cleandown ? <<<<<<<<<<<<<
*>
...
*>
*> option 6 is a special to cleardown all data - May need a update to this
*>                                               subject to GL processing.
...
*> *> /MYSQL-END\
*>
     move    "OPEN GLBATCH" to WS-File-Key
...
*>                    TABLE=GLBATCH-REC
           ...
           STRING "SELECT * FROM "
             "`GLBATCH-REC`"
             " WHERE "
             ws-Where (1:J)      *> BATCH-KEY > "000000" ORDER BY GLBATCH-REC ASC
           ...
           MOVE WS-MYSQL-RESULT TO TP-GLBATCH-REC
*>               /MYSQL-END\
...
*>
     if       WS-MYSQL-Count-Rows = zero
              ...
              if    WS-MYSQL-Error-Number  not = "0  "
                    ...
                    initialize WS-Batch-Record with filler
...
*>
     move     HV-BATCH-KEY to WS-File-Key.
...
*>
     ...
     move     HV-BATCH-KEY to ws-temp-ed.
...
*>
     ...
     move     WS-Where (1:J)  to WS-Log-Where.    *>  For test logging
     move     WS-Batch-Record (K:L) to WS-File-Key.
     if    Testing-2
           display Display-Message-1 with erase eos
     end-if
*>
...
*>
 ba070-Process-Write.
     perform  bb000-HV-Load.                       *>  move WS-Batch-Record fields to HV fields
...
*>
     ...
     string   "Deleting back from " delimited by size
               ws-BATCH-KEY             delimited by size
                      into WS-File-Key
...
*>
     ...
     move     Cleared-Status     to HV-CLEARED-STATUS
     move     Bcycle             to HV-BCYCLE
     move     Entered            to HV-ENTERED
     move     Proofed            to HV-PROOFED
     move     Posted             to HV-POSTED
     move     Stored             to HV-STORED
     move     Input-Gross        to HV-INPUT-GROSS
     move     Input-Vat          to HV-INPUT-VAT
     move     Actual-Gross       to HV-ACTUAL-GROSS
     move     Actual-Vat         to HV-ACTUAL-VAT
     move     Description        to HV-DESCRIPTION
     move     bDefault           to HV-BDEFAULT
     move     Convention         to HV-CONVENTION
     move     Batch-Def-AC       to HV-BATCH-DEF-AC
     move     Batch-Def-PC       to HV-BATCH-DEF-PC
     move     Batch-Def-Code     to HV-BATCH-DEF-CODE
     move     Batch-Def-Vat      to HV-BATCH-DEF-VAT
...
*>
     ...
     move     HV-ITEMS          to Items
     move     HV-BATCH-STATUS   to Batch-Status
     move     HV-CLEARED-STATUS to Cleared-Status
     move     HV-BCYCLE         to Bcycle
     move     HV-ENTERED        to Entered
     move     HV-PROOFED        to Proofed
     move     HV-POSTED         to Posted
     move     HV-STORED         to Stored
     move     HV-INPUT-GROSS    to Input-Gross
     move     HV-INPUT-VAT      to Input-Vat
     move     HV-ACTUAL-GROSS   to Actual-Gross
     move     HV-ACTUAL-VAT     to Actual-Vat
     move     HV-DESCRIPTION    to Description
     move     HV-BDEFAULT       to bDefault
     move     HV-CONVENTION     to Convention
     move     HV-BATCH-DEF-AC   to Batch-Def-AC
     move     HV-BATCH-DEF-PC   to Batch-Def-PC
     move     HV-BATCH-DEF-CODE to Batch-Def-Code
     move     HV-BATCH-DEF-VAT  to Batch-Def-Vat
...
*>
           STRING '`BATCH-KEY`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-BATCH-KEY
             TO WS-MYSQL-EDIT
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`ITEMS`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-ITEMS
             TO WS-MYSQL-EDIT
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BATCH-STATUS`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-BATCH-STATUS
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(18:03))
             INTO WS-MYSQL-COMMAND
             WITH POINTER WS-MYSQL-I end-string
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`CLEARED-STATUS`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-CLEARED-STATUS
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(18:03))
             INTO WS-MYSQL-COMMAND
             WITH POINTER WS-MYSQL-I end-string
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BCYCLE`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-BCYCLE
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(18:03))
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`ENTERED`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-ENTERED
             TO WS-MYSQL-EDIT
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`PROOFED`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-PROOFED
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(11:10))
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`POSTED`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-POSTED
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(11:10))
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`STORED`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-STORED
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(11:10))
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`INPUT-GROSS`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-INPUT-GROSS
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(09:12))
             INTO WS-MYSQL-COMMAND
             WITH POINTER WS-MYSQL-I end-string
           STRING "." INTO WS-MYSQL-COMMAND
             WITH POINTER WS-MYSQL-I end-string
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`INPUT-VAT`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-INPUT-VAT
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(09:12))
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`ACTUAL-GROSS`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-ACTUAL-GROSS
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(09:12))
             INTO WS-MYSQL-COMMAND
             WITH POINTER WS-MYSQL-I end-string
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`ACTUAL-VAT`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-ACTUAL-VAT
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(09:12))
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`DESCRIPTION`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-DESCRIPTION,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ', 'INTO WS-MYSQL-COMMAND
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BDEFAULT`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-BDEFAULT
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(18:03))
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`CONVENTION`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-CONVENTION,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ', 'INTO WS-MYSQL-COMMAND
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BATCH-DEF-AC`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-BATCH-DEF-AC
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(13:08))
             INTO WS-MYSQL-COMMAND
             WITH POINTER WS-MYSQL-I end-string
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BATCH-DEF-PC`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-BATCH-DEF-PC
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(18:03))
             INTO WS-MYSQL-COMMAND
             WITH POINTER WS-MYSQL-I end-string
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BATCH-DEF-CODE`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-BATCH-DEF-CODE,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ', 'INTO WS-MYSQL-COMMAND
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BATCH-DEF-VAT`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-BATCH-DEF-VAT,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ', 'INTO WS-MYSQL-COMMAND
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`BATCH-START`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-BATCH-START
             TO WS-MYSQL-EDIT
           STRING FUNCTION TRIM (WS-MYSQL-EDIT(13:08))
