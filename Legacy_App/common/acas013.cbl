...
*>**
*> Remarks.             Sales/Purch Value File Handler.
*>                      ******************
...
*>                      AC902 Program Error: Temp rec =yyy < Value-Rec = zzz
*>**
...
*> 24/07/16 vbc       acas013 created from acas011 source with same versioning.
*> 27/07/16 vbc - .04 Rem'd out some mv zero to fs-reply but don't think it is causing issues, may be
*> 24/10/16 vbc - .   ALL programs now using wsnames.cob in copybooks
*> 30/10/16 vbc - .05 Remove Dup processing.
*> 15/01/18 vbc - .06 Added support for fn-Delete-All pre call to valueMT as open o/p.
...
*>    acas013 ONLY             WS-Value-Record = Contents of data record to be written/read
...
*>**********************
*>
 77  prog-name           pic x(17)    value "acas013 (3.02.10)".
*>
...
*>   although we could use the one in the copybook for ALL systems its just a
 ...
 *>                  close Value-File
       ...
       if     fn-i-o
              ...
              if       fs-reply not = zero
                       close       Value-File
                       open output Value-File           *> Doesnt create in i-o
                       close       Value-File
                       open i-o    Value-File
              end-if
       else
        if    fn-output
              open output Value-File         *> caller should check fs-reply
        else
         if   fn-extend                      *> Must not be used for ISAM files
*>              open extend Value-File
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     ...
     move    "OPEN VALUE File" to WS-File-Key
     if       fs-reply not = zero
...
*>
     if       File-Key-No = 1
        and   fn-equal-to
              start Value-File key = VA-Code invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
     ...
     if       File-Key-No = 1
        and   fn-not-less-than
              start Value-File key not < VA-Code invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
...
*>
 aa080-Process-Delete.
     ...
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status
     delete   Value-File record invalid key
              move 21 to FS-Reply
     end-delete.
     go       to aa999-main-exit.
*>
...
*>
     ...
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status
     rewrite  Value-Record invalid key
              move 21 to FS-Reply
     end-rewrite
     go       to aa999-main-exit.
*>
