...
*>********************************************
*>                                           *
*>  DelInvNos Table Load via the DelInvNos   *
*>                  file                     *
*>                 for MySQL                 *
*>              Sales  Ledgers               *
*>-------------------------------------------*
...
*>**
*>   Remarks.            Delinvnos RDB Table load from the Delinvnos File
*>                       using the Mysql RDBMS.
*>                       For use with ACAS v3.02 and later only.
*>**
*>   Version.            See prog-name in Ws.
*>**
*>   Called Modules.     acas017   } For file and table access.
*>                       sldelinvnosMT }
*>                       fhlogger
*>
*>**
...
*> 02/07/23 vbc - .14 See UPDATE note in Remarks regarding acas.param.
...
*>----------------------
 77  prog-name           pic x(23)    value "sldelinvnosLD (3.02.15)".
...
*> Program specific  Used : 10, 11, 12, 13, 14, 18, 97, 98, 99
     ...
     03  SL011          pic x(39) value "SL011 Error on sldelinvnosMT processing".
     03  SL012          pic x(37) value "SL012 Error opening Delinvnos File = ".
     ...
     03  SL097          pic x(31) value "SL097 No Delinvnos file present".
     03  SL098          pic x(15) value "SL098 Rewriting".
...
*>
 aa010-Proc-Override.
*>
*>  Over-ride processes for acas017/sldelinvnosMT for Dup modes
*>
...
*>
     ...
     perform  acas017-Open-Input.      *> open input Cobol Delinvnos FILE
     if       FS-Reply = 35
     ...
     if       FS-Reply not = zero
              ...
              close System-File
              perform acas017-Close
              move 16 to return-code    *> data file problem
              goback.
*>
     move     1 to File-System-Used
                   FA-File-System-Used.            *> RDB in use
     perform  acas017-Open.                     *> Open/Init RDB via FH - glDelinvnos
...
*>
 aa999-Finish.
     ...
     perform  acas017-Close.                     *> Close RDB
     close    System-File.
     move     zero to File-System-Used
                   FA-File-System-Used.
     perform  acas017-close.    *> Close cobol Delinvnos file * logging
     move     "EOJ - Load DelInvNos Table" to So-Print.
     perform  Call-Sysout.
     move     "CLOSE   " to SO-Print.
     perform  Call-Sysout.
     goback.
