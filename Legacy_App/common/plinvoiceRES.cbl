...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the pInvoice    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 pInvoiceRES source taken from analysisRES.
...
*>
 copy "plselpinv.cob".
     select  pInvoice-File-Seq  assign        "pinvoice.seq"
...
*>
 copy "plfdpinv.cob".
 fd  pInvoice-File-Seq.
 copy "plwspinv2.cob" replacing PInvoice-Header by WS-PInvoice-Record.
*>
 working-storage section.
*>----------------------
 77  prog-name           pic x(22)    value "pInvoiceRES (3.02.00)".
...
*>
 aa000-Main-Start.
     ...
     move     6      to WS-Log-System.   *> 1 = IRS, 2=GL, 3=SL, 4=PL, 5=Stock used in FH logging, 6=SL & PL
...
*>
     open     input    pInvoice-File-Seq.
     if       FS-Reply not = zeros
              move     "No pInvoice file to process" to SO-Print
              perform  Call-Sysout
              close    Invoice-File
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open pInvoice file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     pInvoice-File-seq next record at end
              ...
              write    Invoice-Record from WS-PInvoice-Record
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_pInvoiceS now
*>
 Call-Sysout.
