...
*>********************************************
*>                                           *
*>  Delivery Table Load via the Delivery     *
*>                  file                     *
*>                 for MySQL                 *
...
*>**
*>   Remarks.            Delivery RDB Table load from the Delivery File
*>                       using the Mysql RDBMS.
*>                       For use with ACAS v3.02 and later only.
*>**
*>   Version.            See prog-name in Ws.
*>**
*>   Called Modules.     acas014   } For file and table access.
*>                       deliveryMT }
*>                       fhlogger
*>
*>**
...
*> 31/05/23 vbc - .14 Chng WS-Log-File-No 12 to 14.
...
*>----------------------
 77  prog-name           pic x(22)    value "deliveryLD (3.02.16)".
...
*> Program specific  Used : 10, 11, 12, 13, 14, 18, 97, 98, 99
     ...
     03  SY011          pic x(39) value "SY011 Error on deliveryMT processing".
     03  SY012          pic x(36) value "SY012 Error opening Delivery File = ".
     03  SY013          pic x(30) value "SY013 Error on reading file = ".
     03  SY014          pic x(31) value "SY014 Error on writing table = ".
     03  SY015          pic x(37) value "SY015 Error opening Delivery Table = ".
     03  SY018          pic x(22) value "SY018 RDBMS details : ".
     03  SY097          pic x(30) value "SY097 No Delivery file present".
     03  SY098          pic x(15) value "SY098 Rewriting".
...
*>
 aa000-main-start.
     ...
     perform  Call-Sysout.
     move     6      to WS-Log-System.   *> 1 = IRS, 2=GL, 3=SL, 4=PL, 6=PL & SL, 5=Delivery used in FH logging
     move     14     to WS-Log-File-No.  *> RDB, File/Table within sub System
*>
...
*>
 aa010-Proc-Override.
*>
*>  Over-ride processes for acas014/deliveryMT for Dup modes
*>
...
*>
     perform  acas014-Open-Input.      *> open input Cobol Delivery FILE
     if       FS-Reply = 35
              move Prog-Name to SO-Print.
              perform call-Sysout
...
*>
     ...
     perform  acas014-Open.                     *> Open/Init RDB via FH - Delivery
     if       FS-Reply not = zero
              move   SY015 to SO-Print
              perform call-Sysout
              move "CLOSE   " to SO-Print
              perform call-Sysout
              close System-File
...
*>
 aa999-Finish.
     ...
     perform  acas014-Close.                     *> Close RDB
     close    System-File.
     move     zero to File-System-Used
                   FA-File-System-Used.
     perform  acas014-close.    *> Close cobol Delivery file * logging
     move     "EOJ - Load Delivery Table" to So-Print.
     perform  Call-Sysout.
     move     "CLOSE   " to SO-Print.
     perform  Call-Sysout.
     goback.
