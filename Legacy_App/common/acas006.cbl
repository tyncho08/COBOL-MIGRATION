...
*>
*>^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
*>  CODING NEEDS CHECKING ONCE GL5* HAS BEEN WORKED ON FOR RELATIVE proc.
*> STOPPED AT 050 as Cobol file is relative and need to closely
*>   inspect to consider converting to ISAM along with the batch file.
*> NOW ISAM on POST-RRN key
*>^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
*>
...
*>**
*> Remarks.             GL Posting File Handler.
*>                      **************
...
*> 04/01/17 vbc -     Taken from acasirsub4 (irsposting)
...
*>                    On write invalid key just mv 22 to reply and quit
*>                    MAY NEED CHANGING subject to GL processing.
...
*>    acas006 ONLY          WS-Posting-Record = Contents of data record to be written/read
...
*>**********************
*>
 77  prog-name          pic x(20)  value "acas006 (3.02.09)".
*>
...
*>
     ...
     move     12     to WS-Log-File-No.  *> RDB, File/Table within sub System.
*>
...
*>              open extend Posting-File
     ...
     move     "OPEN GL POSTING file" to WS-File-Key.
     if       fs-reply not = zero
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     move     zero to Cobol-File-Status.
     move     "CLOSE GL POSTING file" to WS-File-Key.
...
*>
 aa041-Reread.
     ...
     if       FS-Reply not = zero
              go to aa999-main-exit.
     move     Posting-Record to  WS-Posting-Record.
...
*>  USING POST-RRN
*>
*>     We are reading/writing/deleting RRN relative <<<<<<<<<<<<<<
*>
 aa051-Reread.
     move     WS-Post-rrn    to Post-Rrn.    *> USING THIS ???   <<<<
     move     WS-Post-Key to Post-Key WS-File-Key.
     move     zero to Cobol-File-Status.
...
*>
     ...
     move     WS-Post-Key to WS-File-Key
                        Post-Key.
     move     WS-Post-rrn to Post-Rrn.    *> relatives  ??  <<<<<
*>
...
*>
     if       fn-equal-to
              start Posting-File key = Post-Rrn invalid key  *> Post-Key invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
     if       fn-not-less-than
              start Posting-File key not < Post-Rrn invalid key  *> Post-Key invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
...
*>
 aa070-Process-Write.
     ...
     move     WS-Posting-Record to Posting-Record.
...
*>
 aa080-Process-Delete.
     ...
     move     WS-Post-Key to Post-Key.
     move     WS-Post-Key to WS-File-Key.
     move     WS-Post-rrn to Post-rrn.       *>   NEW for relative ?? <<
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
*>
...
*>
     if       A = zero                        *> so it is being called first time
              ...
              if       WE-Error = 901                  *> record length wrong so display error, accept and then stop run.
                       ...
                       string GL903          delimited by size
                              A              delimited by size
                              " < "          delimited by size
                              "Posting-Rec = " delimited by size
...
*>
 ba020-Process-DAL.
     call     "glpostingMT" using File-Access
                                  ACAS-DAL-Common-data
