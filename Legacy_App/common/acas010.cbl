...
*>***********************************************
*>                                              *
*>      Stock Audit File/Table Handler          *
*>                                              *
...
*>**
*> Remarks.             Stock Audit File Handler.
*>                      ******************
...
*>
*>        This module acas010 (along with auditMT) are the FH (cobol File Handler for I/O)
...
*>   **************************************** END OF WARNING ********************************
*>
*>                      This modules does all Cobol file handling for this file (Stock File).
*>
...
*>
*>                      Module naming is based on the File ID variable name and for the Stock audit
*>                      file which uses file-10  the FH is therefore acas010 - the last two chars
...
*>                    from here and the auditMT code.
...
*> 20/03/18 vbc - .06 Changed to funct 6 Delete-All when calling auditMT as per acas006.
...
*>    acas010 ONLY             Stock-Audit-Record = Contents of data record to be written/read
...
*>**********************
*>
 77  prog-name           pic x(17)    value "acas010 (3.02.09)".
*>
...
*> Module Specific
     ...
     03  ST902          pic x(32) value "ST902 Program Error: Temp rec = ".
*>                                        yyy < Stock-Audit-Rec = zzz
*>
 Linkage Section.
*>**************
 copy "wsaudit.cob".
...
*>
     evaluate File-Function
              when  4   *> fn-read-indexed        *>  THIS FILE IS REALLY SEQUENTIAL Audit-ID
              when  9   *> fn-start               *>  is only for RDBMS as all recs will be
                if     File-Key-No not = 1        *>   read as READ-NEXT
              ...
              when     8  *> fn-delete
                if     File-Key-No not = 1        *> 1 is only for RDB as Cobol does it on primary key
                       ...
                       move 99 to fs-reply        *>   SEE ABOVE notes
                       go   to aa999-main-exit
                end-if
     end-evaluate.
*>
*>  Special to allow RDB equivilent of Open file as output (taken from acas006)
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     ...
     move    "OPEN Stock Audit File" to WS-File-Key.
     if       fs-reply not = zero
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     move     zero to Cobol-File-Status.
     move     "CLOSE Stock Audit File" to WS-File-Key.
...
*>
     initialise Stock-Audit-Record with filler.    *> was move spaces
     read     Stock-Audit next record at end
     ...
     if       FS-Reply not = zero
              go to aa999-main-exit.
     move     Stock-Audit-Record to ws-Stock-Audit-Record.
     move     WS-Audit-Key to WS-File-Key.
     move     zeros to WE-Error.
     go to    aa999-main-exit.
*>
 aa070-Process-Write.
     move     206 to WS-No-Paragraph.
     move     ws-Stock-Audit-Record to Stock-Audit-Record.
     ...
     if       A = zero                        *> so it is being called first time
              ...
              if       WE-Error = 901                  *> record length wrong so display error, accept and then stop run.
                       ...
                       string ST902          delimited by size
                              A              delimited by size
                              " < "          delimited by size
                              "Stock-Audit-Rec = " delimited by size
...
*>
 ba020-Process-DAL.
     call     "auditMT" using File-Access
                              ACAS-DAL-Common-data
