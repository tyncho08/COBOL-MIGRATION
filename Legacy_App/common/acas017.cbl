...
*>                                              *
*>                   SL DelInvNo                *
*>       (Deleted Invoice number control)       *
*>               File/Table Handler             *
*>                                              *
...
*>**
*> Remarks.             SL DelInvNo File Handler.
*>                      **************
...
*>    acas017 ONLY          WS-Del-Inv-Nos-Record = Contents of data record to be written/read
...
*>**********************
*>
 77  prog-name          pic x(20)  value "acas017 (3.02.09)".
*>
...
*> Module Specific
     ...
     03  SL903          pic x(32) value "SL903 Program Error: Temp rec = ".
*>                                        yyy < DelInvNo-Rec = zzz
*>
 Linkage Section.
*>**************
 copy "wsdnos.cob".
...
*>  For logging only.
*>
     move     3      to WS-Log-System.   *> 1 = IRS, 2=GL, 3=SL, 4=PL, 5=Stock - used in FH logging
 ...
 aa020-Process-Open.
     ...
     if       fn-input
       ...
       if     fn-i-o
              ...
              if       fs-reply not = zero
                       close       Del-Inv-Nos-File
                       open output Del-Inv-Nos-File           *> Doesnt create in i-o
                       close       Del-Inv-Nos-File
                       open i-o    Del-Inv-Nos-File
              end-if
       else
        if    fn-output
              open output Del-Inv-Nos-File       *> caller should check fs-reply
        else
...
*>              open extend Del-Inv-Nos-File
     ...
     move     "OPEN SL DelInvNo file" to WS-File-Key.
     if       fs-reply not = zero
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     move     zero to Cobol-File-Status.
     move     "CLOSE SL DelInvNo file" to WS-File-Key.
...
*>
 aa041-Reread.
     read     Del-Inv-Nos-File next record at end
     ...
     if       FS-Reply not = zero
              go to aa999-main-exit.
     move     Del-Inv-Nos-Record to  WS-Del-Inv-Nos-Record.
     move     Del-Inv-Nos   to WS-File-Key.
...
*>
 aa051-Reread.
     move     WS-Del-Inv-Nos to Del-Inv-Nos WS-File-Key.
     read     Del-Inv-Nos-File    invalid key
...
*>
     if       fn-equal-to
              start Del-Inv-Nos-File key = Del-Inv-Nos invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
     if       fn-not-less-than
              start Del-Inv-Nos-File key not < Del-Inv-Nos invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
...
*>   IS SL/PL USING START if so HOW ????
*>
...
*>
 aa070-Process-Write.
     ...
     move     WS-Del-Inv-Nos-Record to Del-Inv-Nos-Record.
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
     move     Del-Inv-Nos to WS-File-Key.
     write    Del-Inv-Nos-Record invalid key
              go to  aa999-main-exit.   *> logging
     move     Del-Inv-Nos to WS-File-Key.
     go       to aa999-main-exit.
*>
 aa080-Process-Delete.
     ...
     move     WS-Del-Inv-Nos to Del-Inv-Nos.
     move     WS-Del-Inv-Nos to WS-File-Key.
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
*>
...
*>
     move     17 to WS-Log-File-no.        *> for FHlogger
*>
...
*>
     if       A = zero                        *> so it is being called first time
              ...
              if       WE-Error = 901                  *> record length wrong so display error, accept and then stop run.
                       ...
                       string SL903          delimited by size
                              A              delimited by size
                              " < "          delimited by size
                              "Del-Inv-Nos-Rec = " delimited by size
...
*>
 ba020-Process-DAL.
     call     "sldelinvnosMT" using File-Access
                                  ACAS-DAL-Common-data
