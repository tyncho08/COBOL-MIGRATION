...
*>**
*> Remarks.             Final File RDB Handler using amended JC preSQL.
*>                      **********************************************
*>
...
*>                      from the ACAS
*>                      final (as well as default, system-4 and system) file handler acas000
...
*> 07/12/22 vbc - .12 Chgd Vars A & B to pic 999 to keep GC v3.2 happy.
...
*>
 ...
 77  prog-name          pic x(17)    value "finalMT (3.02.12)".
*>
...
*>
*> Metadata on primary and alternate keys...  from Prima DAL for SYSFINAL-REC
*>
 01  Table-Of-Keynames.
     03  filler         pic x(30) value 'FINAL-ACC-REC-KEY             '.
...
*> /MYSQL VAR\
*>       ACASDB
*>       TABLE=SYSFINAL-REC,HV
 COPY "mysql-variables.cpy".
*>
*>    Definitions for the SYSFINAL-REC Table
*>
       01  TP-SYSFINAL-REC                       USAGE POINTER.
       01  TD-SYSFINAL-REC.
           05  HV-FINAL-ACC-REC-KEY              PIC  9(03) COMP.
           05  HV-AR1                            PIC X(16).
*> /MYSQL-END\
...
*> /MYSQL-END\
*>
     move    "OPEN FINAL" to WS-File-Key
...
*>
     ...
     move    "CLOSE FINAL" to WS-File-Key.
*>  /MYSQL CLOSE\
*>
*>    Close the Database
*>
...
*>
     go      to ba999-end.
*>
 ba040-Process-Read-Next.    *> Getting the 26 rows for loading into one cobol rec
*>
 ...
 *> FINAL-ACC-REC-KEY > "000" ORDER BY FINAL-ACC-REC-KEY ASC
*>               /MYSQL SELECT\
*>
*>    Select rows
*>
*>                    TABLE=SYSFINAL-REC
           ...
           MOVE WS-MYSQL-RESULT TO TP-SYSFINAL-REC
*>               /MYSQL-END\
...
*>
*>              TABLE=SYSFINAL-REC
           MOVE TP-SYSFINAL-REC TO WS-MYSQL-RESULT
           CALL "MySQL_fetch_record" USING WS-MYSQL-RESULT
                    HV-FINAL-ACC-REC-KEY
                    HV-AR1
...
*>
              ...
              if       return-code = zero
                 and   HV-FINAL-ACC-REC-KEY = zero or > 26
                       move    "EOF3" to WS-File-Key
                       move    HV-FINAL-ACC-REC-KEY to WE-Error
                       move    zero to Most-Cursor-Set
                       if      Testing-1                                      *> do for each row
                               perform Ca-Process-Logs
                       end-if
...
*>
              move     HV-FINAL-ACC-REC-KEY to WS-File-Key
              if       WS-MYSQL-Count-Rows = zero
                       call  "MySQL_errno" using WS-MYSQL-Error-Number
              ...
              move     HV-AR1       to AR1 (HV-FINAL-ACC-REC-KEY)     *> KEY = table position
              if       Testing-1
                       perform Ca-Process-Logs
...
*>
     move     HV-FINAL-ACC-REC-KEY to WS-File-Key.    *> more of a check but will be 26
...
*>
     ...
     perform  varying A from 1 by 1 until A > 26
              ...
              move     A       to HV-FINAL-ACC-REC-KEY
              move     AR1 (A) to HV-AR1
...
*>                       HV-FINAL-ACC-REC-KEY  delimited by size                   *>  use the real key for row
...
*>
           STRING '`FINAL-ACC-REC-KEY`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           MOVE HV-FINAL-ACC-REC-KEY
             TO WS-MYSQL-EDIT
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`AR1`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-AR1,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ";" INTO WS-MYSQL-COMMAND
