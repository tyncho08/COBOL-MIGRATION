...
*>**
*> Remarks.             Sales/Purch Analysis File Handler.
*>                      ******************
...
*>
*>        This module acas015 (along with analMT) are the first FH (cobol File Handler for I/O)
...
*>                      AC904 Program Error: Temp rec =yyy < Stock-Rec = zzz
*>**
...
*> 24/07/16 vbc       acas015 created from acas011 source with same versioning.
...
*>                    and removed msgs 906 & 7 renamed 901/2 to AC901/2.
*> 29/04/18 vbc - .06 Resetting Cobol-File-Status for write,rewrite,delete, start etc
*> 02/07/20 vbc - .07 For Start copy ws-PA-Code to PA-Code (was missing).
...
*>    acas022 ONLY             WS-Analysis-Record = Contents of data record to be written/read
...
*>**********************
*>
 77  prog-name           pic x(17)    value "acas015 (3.02.09)".
*>
...
*> Module Specific
     ...
     03  AC904          pic x(32) value "AC904 Program Error: Temp rec = ".
*>                                        yyy < Purch-Rec = zzz
*>
 Linkage Section.
*>**************
 ...
 aa020-Process-Open.
     ...
     if       fn-input
       ...
       if     fn-i-o
              ...
              if       fs-reply not = zero
                       close       Analysis-File
                       open output Analysis-File           *> Doesnt create in i-o
                       close       Analysis-File
                       open i-o    Analysis-File
              end-if
       else
        if    fn-output
              open output Analysis-File         *> caller should check fs-reply
        else
         if   fn-extend                      *> Must not be used for ISAM files
*>              open extend Analysis-File
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     ...
     move     "OPEN ANALYSIS File" to WS-File-Key.
     if       fs-reply not = zero
...
*> 27/07/16 16:30     move     zeros to FS-Reply WE-Error.
     move     zero to Cobol-File-Status.
     move     "CLOSE ANALYSIS File" to WS-File-Key
...
*>
     ...
     if       FS-Reply not = zero
              go to aa999-main-exit.
     move     Analysis-Record to  WS-Analysis-Record.
     ...
     evaluate File-Function      *> Set up keys just for logging
              ...
              when  9             *> fn-start
                    evaluate  File-Key-No
                              when   1
                                     move   WS-PA-Code       to WS-File-Key
                                                                  PA-Code
                              when   other
...
*>
     ...
     move     PA-Code to WS-File-Key.
     if       File-Key-No = 1
              read     Analysis-File key PA-Code       invalid key
                       move 21 to we-error fs-reply
              end-read
              if       fs-Reply = zero
                       move     Analysis-Record to  WS-Analysis-Record
              else
                       move     spaces     to  WS-Analysis-Record
              end-if
              go       to aa999-main-exit
     end-if.
...
*>
     ...
     if       File-Key-No = 1
        and   fn-not-less-than
              start Analysis-File key not < PA-Code invalid key
                    move 21 to Fs-Reply
                    go to aa999-main-exit
              end-start
     end-if
...
*>
 aa070-Process-Write.
     ...
     move     zero to Cobol-File-Status.
     move     PA-Code to WS-File-Key.
     write    Analysis-Record invalid key
              move 22 to FS-Reply
     end-write.
     go       to aa999-main-exit.
*>
 aa080-Process-Delete.
     ...
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
     delete   Analysis-File record invalid key
              move 21 to FS-Reply
     end-delete.
     go       to aa999-main-exit.
*>
...
*>
     ...
     move     zeros to FS-Reply  WE-Error.
     move     zero to Cobol-File-Status.
     rewrite  Analysis-Record invalid key
              move 21 to FS-Reply
     end-rewrite
     go       to aa999-main-exit.
*>
...
*>
     if       A = zero                        *> so it is being called first time
              ...
              if       WE-Error = 901                  *> record length wrong so display error, accept and then stop run.
                       ...
                       string AC904          delimited by size
                              A              delimited by size
                              " < "          delimited by size
                              "Analysis-Rec = " delimited by size
