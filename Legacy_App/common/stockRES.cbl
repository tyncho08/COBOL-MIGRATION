...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the stock    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 stockRES source taken from analysisRES.
...
*>
 copy "selstock.cob".
     select  Stock-File-Seq     assign       "stockctl.seq"
...
*>----------------------
 77  prog-name           pic x(24)    value "stockRES (3.02.00)".
...
*>
 01  Error-Messages.
     ...
     03  SY006          pic x(31) value "SYS06 Write error on stock file".
...
*>
 aa000-Main-Start.
     ...
     move     5      to WS-Log-System.   *> 1 = IRS, 2=GL, 3=SL, 4=PL, 5=Stock used in FH logging, 6=SL & PL
...
*>
     open     input    Stock-File-Seq.
     if       FS-Reply not = zeros
              move     "No Stock Seq file to process" to SO-Print
              perform  Call-Sysout
              close    Stock-File-Seq
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open - Stock file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout

              move     File-11 to so-print
              perform  Call-Sysout
...
*>
 aa010-Read-Recs.
     perform  forever
              read     Stock-File-Seq next record at end
              ...
              write    Stock-Record from WS-Stock-record
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_stockS now
*>
 Call-Sysout.
