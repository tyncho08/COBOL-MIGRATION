...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 nominalRES source taken from analysisRES.
...
*>----------------------
 77  prog-name           pic x(22)    value "nominalRES (3.02.00)".
...
*>
     open     input    ledger-File-Seq.
     if       FS-Reply not = zeros
              move     "No GL Ledger file to process" to SO-Print
              perform  Call-Sysout
              close    ledger-File
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open - GL Ledger file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     ledger-File-Seq next record at end
