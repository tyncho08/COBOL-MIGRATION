...
*>   Written.            3 June 2023.
*>**
*>   Remarks.            This module only uses the Invoice    file.
*>
...
*>**
*>   Changes.
*>  3/06/23 vbc - .00 invoiceRES source taken from analysisRES.
...
*>
 copy "slselinv.cob".
     select  Invoice-File-Seq  assign          "invoice.seq"
...
*>----------------------
 77  prog-name           pic x(24)    value "invoiceRES (3.02.00)".
...
*>
     open     input    Invoice-File-Seq.
     if       FS-Reply not = zeros
              move     "No Invoice file to process" to SO-Print
              perform  Call-Sysout
              close    Invoice-File
     ...
     if       FS-Reply not = zeros
              move     spaces to SO-Print
              string   "Failed on open Invoice file = "
                       FS-Reply
                           into SO-Print
              perform  Call-Sysout
              move     "CLOSE " to SO-Print
...
*>
 aa010-Read-Recs.
     perform  forever
              read     Invoice-File-Seq next record at end
              ...
              write    Invoice-Record from WS-Invoice-Record
...
aa020-Finish-Off.
     ...
     goback.
*>
 copy "Proc-Get-Env-Set-Files.cob".  *> Only uses ACAS_InvoiceS now
*>
 Call-Sysout.
