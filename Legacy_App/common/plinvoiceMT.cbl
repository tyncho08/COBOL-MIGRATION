...
*>**
*> Remarks.             Purchase Invoice File RDB Handler using amended JC preSQL.
*>                      **********************************************
*>
...
*>                      The primary (PUINVOICE-REC) has all columns that directly link
*>                      to the Cobol file Header data and
*>                      PUINV-LINES-REC for up to 40 rows for those both using the
...
*>                      from the ACAS
*>                      Purchase Invoice file handler acas026
...
*>**
*> Called by Modules:
*>                      acas026 - Purchase Invoice Cobol Handler.
*>
*>**
*> Error Messages Used.
...
*> 08/01/18 vbc - .08 Taken from acas016.
...
*>                    from 'to 1'. Ditto slinvoiceMT & *autogenMT - in progress - did say
...
*>
*> Metadata on primary keys...   for PUINVOICE-REC
*>
 01  Table-Of-KeyNames.
     03  filler         pic x(30) value 'PINVOICE-KEY                  '.   *> In PUINVOICE-REC
     03  filler         pic x(8)  value '00010010'.  *> offset/length
     03  filler         pic x(3)  value 'STR'.       *> data type
 *>
     03  filler         pic x(30) value 'IL-LINE-KEY'.                      *> In PUINV-LINES-REC
     03  filler         pic x(8)  value '00010010'.  *> offset/length
     03  filler         pic x(3)  value 'STR'.       *> data type
...
*>
 01  RG-Table.
     03  filler  pic x(30) value 'PUINV-LINES-REC'. *> table name
...
*> /MYSQL VAR\
*>       ACASDB
*>       TABLE=PUINVOICE-REC,HV
*>       TABLE=PUINV-LINES-REC,HV1
 COPY "mysql-variables.cpy".
*>
*>    Definitions for the PUINVOICE-REC Table
*>
       01  TP-PUINVOICE-REC                      USAGE POINTER.
       01  TD-PUINVOICE-REC.
           05  HV-PINVOICE-KEY                   PIC X(10).
           05  HV-IH-INVOICE                     PIC  9(10) COMP.
           05  HV-IH-TEST                        PIC  9(03) COMP.
           05  HV-IH-SUPPLIER                    PIC X(7).
...
*>
*>    Definitions for the PUINV-LINES-REC Table
*>
       01  TP-PUINV-LINES-REC                    USAGE POINTER.
       01  TD-PUINV-LINES-REC.
           ...
           05  HV1-IL-DESCRIPTION                PIC X(24).
...
*>                    TABLE=PUINVOICE-REC
           ...
           MOVE WS-MYSQL-RESULT TO TP-PUINVOICE-REC
*>               /MYSQL-END\
...
*>
*>             TABLE=PUINVOICE-REC
           MOVE TP-PUINVOICE-REC TO WS-MYSQL-RESULT
...
*>
*>    Save the pointers (PUINVOICE-REC) [ and we will restore at end ]
*>
...
*>             TABLE=PUINV-LINES-REC
           ...
           PERFORM MYSQL-1210-COMMAND THRU MYSQL-1219-EXIT
           PERFORM MYSQL-1220-STORE-RESULT THRU MYSQL-1239-EXIT
           MOVE WS-MYSQL-RESULT TO TP-PUINV-LINES-REC
*>      /MYSQL-END\
*>
     if       WS-MYSQL-Count-Rows not zero
...
*>           EXEC SQL
*>              DELETE
*>              FROM PUINVOICE-REC WHERE IH-INVOICE = {key value (1:8)}
*>           END-EXEC.
*>
...
*>           EXEC SQL
*>              DELETE
*>              FROM PUINV-LINES-REC WHERE IL-INVOICE = {key value (1:8)}
*>           END-EXEC.
*>
...
*>     "PUINVOICE-REC " DELIMITED BY SIZE
*>     "WHERE `IH-INVOICE`="
*>         key value (1:8)
...
*>
     ...
     move     WS-ih-Order         to HV-IH-ORDER
...
*>
     ...
     move     HV-IH-ORDER         to WS-ih-Order
...
*>
           STRING '`PINVOICE-KEY`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
...
*>
           ...
           WITH POINTER WS-MYSQL-I end-string
*>
           STRING '`IH-SUPPLIER`="' INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING FUNCTION TRIM (HV-IH-SUPPLIER,TRAILING)
                  '"'
                   INTO WS-MYSQL-COMMAND
                   WITH POINTER WS-MYSQL-I end-string
           STRING ', 'INTO WS-MYSQL-COMMAND
...
*>
*>             TABLE=PUINV-LINES-REC
           MOVE TP-PUINV-LINES-REC TO WS-MYSQL-RESULT
           CALL "MySQL_fetch_record" USING WS-MYSQL-RESULT
                    HV1-IL-LINE-KEY
                    HV1-IL-INVOICE
                    HV1-IL-LINE
...
*>
 bc080-Exit.  Exit.
*>
 bc085-Process-Delete-ALL.    *> THIS IS NON STANDARD - NEEDED (pl940?)- Coded/ D.Tested.
*>
...
*>           EXEC SQL
*>              DELETE
*>              FROM PUINV-LINES-REC WHERE 'IL-INVOICE' = WS-ih-Invoice
*>           END-EXEC.
*>
*>  That creates the following code from dbpre
*>
...
*>     "FROM " DELIMITED SIZE
*>     "PUINV-LINES-REC " DELIMITED SIZE
...
*>
*> Loading RG table: PUINV-LINES-REC
*> <<<<  this should be a move from invoice-rec to line rec >>>>> <<>>
*>
     initialize TD-PUINV-LINES-REC.
*>
...
*>
*> Unloading RG table: PUINV-LINES-REC
*>
...
*>
*> End of PUINV-LINES-REC unload... but save the last read line.
*>
